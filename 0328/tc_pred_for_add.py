...
if(lcsv):
    ...
    df_add = pd.read_csv(filepath_or_buffer='tc_add.csv',
                     header=0, sep=',', usecols=[0, 2, 6])
    print(df_add.head())
    print(df_add.index)
    print(df_add.columns)
    
    df_add['Tc'] = df_add['     Tc [K]'].apply(float)
    df_add['P']  = df_add['  P [GPa]'].apply(float)
    df_add['list'] = df_add['formula'].apply(get_parameters)
    df_add['formula'] = df_add['formula'].apply(lambda x: x.strip())
    for i in range(len(get_parameters('H3S'))):
        name = 'prm' + str(i)
        df_add[name] = df_add['list'].apply(lambda x: x[i])
    df_add = df_add.drop(['     Tc [K]', '  P [GPa]', 'list'], axis=1)
    df_add.to_csv("tc_pred.csv")
    print(df_add.head(5))
...
key = 'RF' # 'RR' 'EN' 'LASSO' 'kNN' 'RF' 'GB' 'SVR' 'PLS' 'GPR' 'LWPLS'
...
f_pred, X_pred, y_pred_db = read_fxy_csv(pred_file)
