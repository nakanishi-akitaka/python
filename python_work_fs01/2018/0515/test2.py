...
score='r2'
range_c = [2**j for j in range(-5,11)]
range_e = [2**j for j in range(-10,1)]
range_g = [2**j for j in range(-20,11)]
range_d = [i for i in range(1,3)]

#
# 5a. use pipeline, MinMaxScaler optimization of gamma & c
# {{{
start = time()
print('')
print('')
...
# }}}
#
# 5c. use pipeline, MinMaxScaler optimization of epsilon
# {{{
start = time()
print('')
print('')
...
pipe = Pipeline([
('scaler', MinMaxScaler()),
('svr', SVR(C=200,gamma=3000))
])

# step 2. learning with optimized parameters
# search range
cv = ShuffleSplit(n_splits=10, test_size=0.1, random_state=0)
param_grid = [{'svr__kernel':['rbf'], 'svr__epsilon': range_e}]
grid_search = GridSearchCV(pipe, param_grid, n_jobs=-1, scoring=score,cv=cv)
grid_search.fit(X_train, y_train)
print_search_score(grid_search)
print("learning   score: ",end="")
