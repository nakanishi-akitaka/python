...
def print_high_Tc(X_test,y_pred):
    for i in range(len(X_test)):
        if(y_pred[i]> 150):
            ...
            % (atom0,X_test[i][2],atom1,X_test[i][3],int(X_test[i][22]),y_pred[i]))
#}}}
#
...
for line in trainFile:
    ...
    features.extend(row)
    features.extend(eneg)
    features.extend(maxos)
    features.extend(minos)
    features.extend(amass)
    features.extend(mende)
    features.extend(meltp)
    features.extend(therm)
