...
error = math.sqrt(mean((mean(tc) - tc)**2))
print("RMQE: " + str(round(error, 3)) + " K")
...
# print("MAE by RF with composition data: "+ str(round(abs(mean(sc)), 3)) + " K")
sc = cross_val_score(rfr, naiveFeatures, tc, cv=cv, scoring='neg_mean_squared_error')
print("MSE by RF with composition data: "+ str(round(abs(mean(sc)), 3)) + " K")
for i in range(1,20):
    cv = ShuffleSplit(n_splits=i, test_size=0.1, random_state=0)
    sc = cross_val_score(rfr, naiveFeatures, tc, cv=cv, scoring='neg_mean_squared_error')
    print(" n_splits = " + str(i) + "  MSE by RF with composition data: "+ str(round(abs(mean(sc)), 3)) + " K")
...
sp = cross_val_score(rfr,pf1,tc, cv=cv, scoring='neg_mean_squared_error')
print("MSE by RF with physical 1 data: "+ str(round(abs(mean(sp)), 3)) + " K")
sp = cross_val_score(rfr,pf2,tc, cv=cv, scoring='neg_mean_squared_error')
print("MSE by RF with physical 2 data: "+ str(round(abs(mean(sp)), 3)) + " K")
sp = cross_val_score(rfr,pf3,tc, cv=cv, scoring='neg_mean_squared_error')
print("MSE by RF with physical 3 data: "+ str(round(abs(mean(sp)), 3)) + " K")
sp = cross_val_score(rfr,pf4,tc, cv=cv, scoring='neg_mean_squared_error')
print("MSE by RF with physical 4 data: "+ str(round(abs(mean(sp)), 3)) + " K")
sp = cross_val_score(rfr,pf5,tc, cv=cv, scoring='neg_mean_squared_error')
print("MSE by RF with physical 5 data: "+ str(round(abs(mean(sp)), 3)) + " K")
sp = cross_val_score(rfr,pf6,tc, cv=cv, scoring='neg_mean_squared_error')
print("MSE by RF with physical 6 data: "+ str(round(abs(mean(sp)), 3)) + " K")
sp = cross_val_score(rfr,pf7,tc, cv=cv, scoring='neg_mean_squared_error')
print("MSE by RF with physical 7 data: "+ str(round(abs(mean(sp)), 3)) + " K")
