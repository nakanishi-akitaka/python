...
def print_high_Tc(X_test,y_pred):
    for i in range(len(X_test)):
        if(y_pred[i]> 120):
            ...
            print('%2s%.1i%1s%.1i P = %.3i GPa Tc = %.1f K'             % (atom0,X_test[i][2],atom1,X_test[i][3],int(X_test[i][4]),y_pred[i]))
#}}}
...
#   print('%s%.1i %s%.1i' % (element.from_Z(atomicNo[0]).symbol,natom[0], #                            element.from_Z(atomicNo[1]).symbol,natom[1]))
...
# search range
range_c = [i*10**j for j in range(-2,4) for i in range(1,2)]
range_g = [i*10**j for j in range(-1,2) for i in range(1,10,2)]
