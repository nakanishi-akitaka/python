...
grid_search = GridSearchCV(svr,param_grid, n_jobs=-1, scoring=score)
...
# }}}
#
# 1.1 check best_params
# {{{
scores = cross_val_score(grid_search.best_estimator_,X_train,y_train,scoring=score)
print(scores)
# }}}
#
# 2. use pipeline, Scaler
# {{{
start = time()
print('')
print('')
...
grid_search = GridSearchCV(pipe, param_grid, n_jobs=-1, scoring=score)
grid_search.fit(X_train, y_train)
print_search_score(grid_search)
print("learning   score: ",end="")
print_score(grid_search, X_train, y_train)
...
'''
#
# 3. use pipeline, Scaler, PCA 
# {{{
start = time()
print('')
print('')
print('# SVR with GridSearched hyper parameters after MinMaxScaler and PCA')
...
# step 2. learning with optimized parameters
# search range
range_p =  [i for i in range(1,6)]
param_grid = [
{'pca__n_components':range_p,'svr__kernel':['linear'], 'svr__C':range_c},  
{'pca__n_components':range_p,'svr__kernel':['rbf'],    'svr__C':range_c, 'svr__gamma': range_g},
{'pca__n_components':range_p,'svr__kernel':['sigmoid'],'svr__C':range_c, 'svr__gamma': range_g}]
param_grid = [{'pca__n_components':range_p,'svr__kernel':['rbf'],    'svr__C':range_c, 'svr__gamma': range_g}]
grid_search = GridSearchCV(pipe, param_grid, n_jobs=-1, scoring=score)
grid_search.fit(X_train, y_train)
print_search_score(grid_search)
print("learning   score: ",end="")
